# PROYECTO: VIG.IA - CEREBRO (MULTI-IMAGEN SUPPORT)
# ARCHIVO: Nucleo_Vigia.py
# DESCRIPCIÓN: Backend capaz de procesar lotes de imágenes y generar anexos.
...
class InspectorIndustrial:
    ...
    def analizar_imagen_con_ia(self, api_key, lista_rutas_imagenes, datos_ins, datos_tec):
        """
        Recibe una LISTA de rutas de imágenes y las procesa en conjunto.
        """
        ...
        1. HALLAZGOS VISUALES (Integra lo observado en todas las fotos).
        ...
        try:
            ...
            # Pasamos el prompt y la lista de imágenes (desempaquetada o como lista)
    ...
    def generar_pdf_ia(self, datos, texto_ia, lista_rutas_imagenes):
        ...
        if lista_rutas_imagenes:
            pdf.add_page()
            pdf.set_fill_color(255, 111, 0) # Naranja VIG.IA
            pdf.set_text_color(255, 255, 255)
            ...
            for i, ruta in enumerate(lista_rutas_imagenes):
                if os.path.exists(ruta):
                    # Si nos quedamos sin espacio (página llena), nueva página
                    if y_pos > 220:
                        pdf.add_page()
                        y_pos = 40
                        pdf.set_font('Arial', 'B', 14)
                        pdf.cell(0, 10, " ANEXO FOTOGRÁFICO (Cont.)", 0, 1, 'C')
                        pdf.ln(10)

                    pdf.set_font('Arial', 'I', 10)
                    pdf.cell(0, 10, f"Figura {i+1}: Evidencia capturada en sitio.", 0, 1, 'L')
                    
                    try:
                        # Imagen centrada
                        pdf.image(ruta, x=35, y=y_pos+8, w=140, h=105) # 4:3 aspect ratio aprox
                    except: pass
                    
                    y_pos += 120 # Espacio para la siguiente foto
